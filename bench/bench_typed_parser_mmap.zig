...
// Schema definition (same as bench_typed_parser.zig)
const Rating = struct {
    score: f32,
    max_score: u32,
    verified: bool,
...
pub fn main() !void {
    ...
    // Benchmark: parse and iterate through entire document using mmap
    const start = std.time.nanoTimestamp();

    const DigitalPlatformParser = TypedParser(DigitalPlatform);
    var parser = try DigitalPlatformParser.initWithMmap(allocator, filename);
    defer parser.deinit();
