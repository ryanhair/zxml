...
// Schema definition (same as other typed parser benchmarks)
const Rating = struct {
    score: f32,
    max_score: u32,
    verified: bool,
...
pub fn main() !void {
    ...
    // Benchmark: parse and iterate through entire document using in-memory
    const start = std.time.nanoTimestamp();

    const DigitalPlatformParser = TypedParser(DigitalPlatform);
    var parser = try DigitalPlatformParser.initInMemory(allocator, xml);
    defer parser.deinit();
