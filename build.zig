...
pub fn build(b: *std.Build) void {
    ...
    // Create pull_parser module (includes string_storage internally)
    const pull_parser_mod = b.addModule("pull_parser", .{
        .root_source_file = b.path("src/pull_parser.zig"),
        .target = target,
    });
    ...
    const exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            // root module.
            .imports = &.{
                // Here "zxml" is the name you will use in your source code to
                // import this module (e.g. `@import("zxml")`). The name is
    ...
    // Create tests for pull_parser module
    const pull_parser_tests = b.addTest(.{
        .root_module = pull_parser_mod,
    });

    const run_pull_parser_tests = b.addRunArtifact(pull_parser_tests);
    ...
    test_step.dependOn(&run_pull_parser_tests.step);

    // Create pull_parser example executable
    const pull_parser_example = b.addExecutable(.{
        .name = "pull_parser_example",
        .root_module = b.createModule(.{
            .root_source_file = b.path("examples/pull_parser_example.zig"),
            .target = target,
            .optimize = optimize,
            .imports = &.{
                .{ .name = "pull_parser", .module = pull_parser_mod },
    ...
    b.installArtifact(pull_parser_example);

    // Create run step for pull_parser example
    const run_pull_parser_example_step = b.step("run-pull-example", "Run the pull parser example");
    const run_pull_parser_example = b.addRunArtifact(pull_parser_example);
    run_pull_parser_example.step.dependOn(b.getInstallStep());
    if (b.args) |args| {
        run_pull_parser_example.addArgs(args);
    }
    ...
    const generate_xml = b.addExecutable(.{
        .name = "generate_media_library",
        .root_module = b.createModule(.{
            .root_source_file = b.path("bench/generate_media_library.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });

    b.installArtifact(generate_xml);

    // Create run step for XML generator
    const run_generate_xml_step = b.step("generate-xml", "Generate XML test data");
    const run_generate_xml = b.addRunArtifact(generate_xml);
    run_generate_xml.step.dependOn(b.getInstallStep());
    if (b.args) |args| {
        run_generate_xml.addArgs(args);
    }
    run_generate_xml_step.dependOn(&run_generate_xml.step);

    // Create typed_parser example executable
    const typed_parser_example = b.addExecutable(.{
        .name = "typed_parser_example",
        .root_module = b.createModule(.{
            .root_source_file = b.path("examples/typed_parser_example.zig"),
            .target = target,
            .optimize = optimize,
            .imports = &.{
                .{ .name = "zxml", .module = mod },
    ...
    b.installArtifact(typed_parser_example);

    // Create run step for typed_parser example
    const run_typed_parser_example_step = b.step("run-typed-example", "Run the typed parser example");
    const run_typed_parser_example = b.addRunArtifact(typed_parser_example);
    run_typed_parser_example.step.dependOn(b.getInstallStep());
    if (b.args) |args| {
        run_typed_parser_example.addArgs(args);
    }
